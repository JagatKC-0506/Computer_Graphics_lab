...
def display():
    ...
    # ====== CHANGE INPUT POLYGON COLOR ======
    # Original polygon - shows what user entered
    glColor3f(1, 0, 0)  # Red = input polygon | Change to your color: (1,0,0)=red, (0,1,0)=green
    glBegin(GL_LINE_LOOP)
    for p in polygon:
        glVertex2f(p[0], p[1])
    glEnd()
    ...
    if len(polygon) > 0:
        ...
        # ====== CHANGE CLIPPED POLYGON COLOR ======
        # Clipped polygon - shows result after clipping
        if len(clipped) > 0:
            glColor3f(0, 1, 0)  # Green = clipped result | Change to your color: (1,1,0)=yellow, (1,0,1)=magenta
            glBegin(GL_LINE_LOOP)
            for p in clipped:
                glVertex2f(p[0], p[1])
            glEnd()
...
def main():
    ...
    glClearColor(0, 0, 0, 1)  # RGBA: (0,0,0,1)=black | Try: (1,1,1,1)=white, (0.2,0.2,0.2,1)=dark gray
